...
computeWiener = function(){
  ...
  wiener = matrix(0,nrow = nDays,ncol = 1)
  for (i in 1:nDays){
    ...
    v_length = length(periodDF$value)
    weight = rep(0,v_length)
    for(j in 1:v_length){
      m1 = min(periodDF$value)
      m2 = max(periodDF$value)
      weight[j] = (periodDF$value[j]-m1)/(m2-m1)+0.0001
    }
    E(G)$weight = weight
    wiener [i,1] <- wiener_index(G)
    
    if (i %% 30 == 0) print(paste(i,'out of',nDays,'days processed'))
    
  }
  
  #colname(wiener)<- c("Day","Wiener")
  rowNames<-data.frame(Day=paste0(periodList))
  saveRDS(cbind(rowNames,wiener),file=paste0(wienerDir,'allTokens_wiener.rds'))
...
dataMerge <- function(threshold){
  ...
  addwiener = function(){
    # 
    inputFiles = list.files(wienerDir,paste0("(allTokens).*"))
    df = map_dfr(paste0(wienerDir,inputFiles),function(inputFile){
      read_rds(inputFile)
    })
    df = tibble(
      day = df$Day,
      wiener = log(df$wiener))
    df = df %>%
  ...
  #
  mergeData = function(){
    df = inner_join(ghFeature,rollDepth,wiener1,by="day")
    df = inner_join(df,wiener1,by="day")
...
RFmodel <- function(threshold,repNum){
  ...
  inputfmls = c(M1="~ vertexNum + edgeNum + clusterCoef + openNorm",
                ...
                M4="~ vertexNum + edgeNum + clusterCoef + openNorm + wiener",
                M5="~ vertexNum + edgeNum + clusterCoef + openNorm + wiener + B0",
                M6="~ vertexNum + edgeNum + clusterCoef + openNorm + wiener + B0 + B1",
                M7="~ vertexNum + edgeNum + wiener",
                M8="~ wiener")
